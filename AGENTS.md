...
- ~~`jsconfig.json` excludes `src/lib`, `src/api`, and never includes any of `base44/`~~ — **resolved 18 Sept 2026.** `include` now covers `src/lib/**/*.js`, `src/api/**/*.js`, and `src/components/**/*.jsx` (previously only `.js`, silently missing every JSX component); `src/components/ui` exclude glob fixed to `src/components/ui/**` (the bare directory name wasn't matching, so those files were still being typechecked as root files despite the intent to exclude them); added `noEmit`/`allowImportingTsExtensions` (required together) and `types: ["vite/client"]` (fixes `import.meta.env`). Error count on the widened scope went 483 → 202 by fixing real root causes, not by suppressing: added `@typedef {Object} FieldSemantic` (was referenced in JSDoc across `fieldSemantic.js`/`entityFieldMap.js` but never declared anywhere — a genuine gap, not a formatting issue), added missing JSDoc prop types to `EmptyState`, `StatCard`, `PriorityBadge`, `Badge`, and fixed the shadcn `React.forwardRef` primitives (`button`, `input`, `label`, `select`) whose JSDoc `@param` blocks were attached to the outer `const X = React.forwardRef(...)` statement instead of the inner function expression passed to `forwardRef` — TS silently ignores JSDoc placed on the wrong node, so every consumer of these primitives saw an inferred `{}` props type. Also found and fixed a real bug this exposed: `base44/shared/core/ontology/domains/sales.ts` had `es`/`pt`/`de`/`it`/`nl` object keys duplicated in `SALES_CONCEPTS["sales.revenue.net"].synonyms` — the second, larger set silently overwrote the first at runtime (`TS1117`), discarding synonym terms; merged into one set per language. Remaining ~202 errors are pre-existing, unrelated debt spread across many files (`fake-framer-motion.jsx`, `image.jsx`/`responsive-image.jsx`, several page-level prop mismatches) — not touched, since fixing them isn't part of what was asked and would risk unrelated regressions; triage by root cause if picked up next.
